...
pub async fn process_buy_request(
    ...
    let amount_lamports = (request.sol_quantity * 1_000_000_000.0) as u64;
    let slippage_bps = (request.slippage_tolerance * 10_000.0) as u32;

    // Get quote
    let quote_request = QuoteRequest {
        input_mint: WSOL_ADDRESS.to_string(),
    ...
    match confirm_transaction(rpc_client, &signature, 20, 3).await {
        Ok(true) => {
            ...
            let token_quantity = route.out_amount.parse::<f64>().unwrap_or(0.0) / 1e9;
